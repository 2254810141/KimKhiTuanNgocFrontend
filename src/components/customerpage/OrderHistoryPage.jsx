  ...
  if (!isAuthenticated) {
    return (
      <section className="container-app space-y-4 rounded-3xl border border-dashed border-zinc-300 bg-white p-6 text-center shadow-sm sm:p-8">
        <h1 className="text-xl font-black text-zinc-900 sm:text-2xl">Lịch sử đặt hàng</h1>
        ...
        <div className="flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <Link
            to="/products"
            className="inline-flex w-full items-center justify-center rounded-lg border border-zinc-300 px-5 py-3 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-100 sm:w-auto"
          >
            Mua sắm tiếp
          </Link>
        </div>
  ...
  return (
    <section className="container-app space-y-4 sm:space-y-6">
      <div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <h1 className="mt-2 text-2xl font-black text-zinc-900 sm:text-3xl">Lịch sử đặt hàng</h1>
              ...
              <article key={order.orderId} className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
                <div className="flex flex-col gap-4 border-b border-zinc-200 p-4 sm:p-5 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="text-base font-black text-zinc-900 sm:text-lg">Đơn hàng #{order.orderId}</h2>
                  ...
                  <div className="text-left lg:text-right">
                    ...
                    {canCancel && (
                      <button
                        ...
                        className="mt-3 w-full rounded-lg border border-amber-200 px-4 py-2 text-sm font-semibold text-amber-700 transition hover:bg-amber-50 sm:w-auto"
                      >
                        Hủy đơn hàng
                      </button>
                    )}
                ...
                <div className="grid gap-3 p-4 sm:p-5 md:grid-cols-2 xl:grid-cols-3">
                  {order.items?.map((item) => (
                    <div key={`${order.orderId}-${item.productId}`} className="flex flex-col gap-3 rounded-2xl border border-zinc-200 p-3 sm:flex-row sm:items-center">
